...
const getStages = (data) => {
    if (Array.isArray(data)) {
        ...
        const result = {
            newStage: {
                count: temp.filter(
                    (item) => item.stage_code == STAGE1 && !item.win_date && !item.lost_date
                ).length,
                total: newStage,
                title: 'NEW',
            ...
            wipStage: {
                count: temp.filter(
                    (item) => item.stage_code == STAGE2 && !item.win_date && !item.lost_date
                ).length,
                total: wipStage,
            ...
            commitmentStage: {
                count: temp.filter(
                    (item) => item.stage_code == STAGE3 && !item.win_date && !item.lost_date
                ).length,
...
const getProductPerType = (data) => {
    if (Array.isArray(data)) {
        ...
        const result = Object.entries(groupedData).map(([cat_desc, counts]) => ({
            [cat_desc]: counts
        }))
        console.log(result)
